...
var (
	...
	ErrInvalidConfirmationCode = NewDomainError("invalid confirmation code", nil).WithType(Validation)
	ErrPasswordTooLong         = NewDomainError("password exceeds maximum length", nil).WithType(Validation)
	ErrPasswordNotAcceptable   = NewDomainError("password contains forbidden characters", nil).WithType(Validation)
	ErrWeakPassword            = NewDomainError("password is too weak", nil).WithType(Validation)
	ErrCompromisedPassword     = NewDomainError("password is known to be compromised", nil).WithType(Validation)
)
